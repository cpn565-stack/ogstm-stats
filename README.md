...
- **即時彙整儀表板**：跨組加總、贊成率視覺化
- **講師視圖**：大字體投影介面，顯示各題最多人贊成／反對的選項，自動更新
- **後台歷史統計**：保留各場次各組選項明細，支援跨場次歷史加總與 CSV 匯出
- **CSV 匯出**：課後分析用
...
所有場次資料儲存在伺服器 `data/store.json`（部署時需確保此目錄可寫入且定期備份）。

| 路徑 | 用途 |
|------|------|
| `/admin` | 後台：各場次列表、跨場次歷史統計 |
| `/admin/sessions/{id}` | 單場次各組選項明細 |
| `/api/export/all` | 匯出全部歷史 CSV |
